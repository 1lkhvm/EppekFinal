...
function AdminPasta() {
    ...
    function DataGet() {
        axios.get("http://localhost:5000/api/productpasta/").then((response) => {
    ...
    function Datadelete(id) {
        axios.delete(`http://localhost:5000/api/productpasta//${id}`).then(DataGet, DataStat)

    }


    function Dataedit(id) {
        axios.put(`http://localhost:5000/api/productpasta//${id}`).then((response) => {
            setPost(response.data);
        });
                    ...
                    <tbody>
                        {post.map((element) =>
                            <tr>
                                ...
                                <th scope="noBorder"> <Link to={`/product_pasta_admin_panel/${element._id}`} ><button className='button-64' onClick={() => Dataedit()}>Edit</button></Link></th>
